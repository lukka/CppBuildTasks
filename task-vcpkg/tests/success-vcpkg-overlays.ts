...
const answers: ma.TaskLibAnswers = {
  ...
  'exec': {
    ...
    [`${path.join(vcpkgRoot, "vcpkg")} install --recurse vcpkg_args @${responseFile} --overlay-ports=inlineport --triplet triplet`]:
...
vcpkgUtilsMock.utilsMock.readFile = (file: string): [boolean, string] => {
  ...
  else if (file === path.join(vcpkgRoot, responseFile)) {
    return [true, "\n--overlay-ports=lua\n--overlay-ports=../another/port\nport"];
  }
  else
...
tmr.setInput(globals.vcpkgArguments, 'vcpkg_args @respfile --overlay-ports=inlineport');
